...
const Track = ({ sequence, note, pattern, setPattern, label = "drum" }) => {
	...
	return (
		<div
			className={`flex gap-1 sm:gap-2 items-center group ${
				muted ? "opacity-50" : ""
			}`}
		>
			<div className="w-10 sm:w-12 md:w-24 h-4 sm:h-6 md:h-8 lg:h-12 text-sm leading-tight bg-slate-300 p-1 rounded-sm flex items-center justify-between">
				<p className="hidden md:block">{label}</p>
				<p className="uppercase text-xs text- sm:text-sm md:hidden">
					{abbreviateLabel(label)}
				</p>
				<button
					type="button"
					onClick={() => setMuted(!muted)}
					className="px-1 shrink-0 text-slate-700"
				>
					{muted ? (
						<FaVolumeMute size="clamp(10px, 1.5vw, 18px)" />
					) : (
						<FaVolumeOff size="clamp(10px, 1.5vw, 15px)" />
					)}
				</button>
			</div>
			{pattern.map((beat, i) => (
